...
TEST(TestMideaACClass, HumanReadableOutput) {
  ...
  ac.setSleep(true);
  EXPECT_EQ(
      "Power: Off, Mode: 1 (DRY), Celsius: Off, Temp: 25C/77F, Fan: 3 (HI), "
      "Sleep: On", ac.toString());
  ac.setUseCelsius(true);
  EXPECT_EQ(
      "Power: Off, Mode: 1 (DRY), Celsius: On, Temp: 25C/77F, Fan: 3 (HI), "
      "Sleep: On", ac.toString());

  ac.setRaw(0xA19867FFFF7E);
  EXPECT_EQ(
      "Power: On, Mode: 0 (COOL), Celsius: Off, Temp: 21C/69F, Fan: 3 (HI), "
      "Sleep: Off", ac.toString());
}
